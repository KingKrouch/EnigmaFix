/**
Memory Utilities code that is from Lyall's C++ mods (https://github.com/Lyall/). Simply borrowing them here.
...
namespace Util
{
    auto findStringInVector = [](string& str, const initializer_list<string>& search) -> int {
        transform(str.begin(), str.end(), str.begin(),
            [](unsigned char c) { return tolower(c); });

        auto it = find(search.begin(), search.end(), str);
        if (it != search.end())
            return distance(search.begin(), it);
        return 0;
        };
